...
describe('BudgetProgressBar', () => {
  ...
  it('calculates percentage correctly', () => {
    render(
      <BudgetProgressBar name="Rent" spent={1500} budget={1500} baseCurrency={CurrencyCode.USD} />
    );
  ...
  it('handles zero budget', () => {
    render(
      <BudgetProgressBar name="Test" spent={100} budget={0} baseCurrency={CurrencyCode.USD} />
    );
  ...
  it('applies success color for expense under 80%', () => {
    const { container } = render(
      <BudgetProgressBar name="Food" spent={300} budget={500} baseCurrency={CurrencyCode.USD} />
    );
  ...
  it('applies warning color for expense between 80-100%', () => {
    const { container } = render(
      <BudgetProgressBar name="Food" spent={450} budget={500} baseCurrency={CurrencyCode.USD} />
    );
    const progressBar = container.querySelector('.MuiLinearProgress-bar');
    expect(progressBar).toBeInTheDocument();
  ...
  it('applies error color for expense over 100%', () => {
    const { container } = render(
      <BudgetProgressBar name="Food" spent={600} budget={500} baseCurrency={CurrencyCode.USD} />
    );
    const progressBar = container.querySelector('.MuiLinearProgress-bar');
    expect(progressBar).toBeInTheDocument();
