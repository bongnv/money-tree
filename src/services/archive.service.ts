...
export class ArchiveService {
  ...
  async calculateYearEndSummary(year: number, baseCurrency: CurrencyCode): Promise<YearEndSummary> {
    ...
    const closingNetWorth = await this.calculationService.calculateNetWorth(
      accounts,
      transactions,
      manualAssets,
      baseCurrency,
    ...
    for (const account of accounts) {
      const balance = this.calculationService.calculateAccountBalance(account, yearTransactions);
   ...
   */
  updateMainFileAfterArchive(year: number, archiveReference: ArchivedYearReference): void {
    ...
    accountStore.accounts.forEach((account) => {
      ...
      const balance = this.calculationService.calculateAccountBalance(
        account,
   ...
   */
  getArchivedYears(): ArchivedYearReference[] {
    const appState = useAppStore.getState();
